...
class ViewController: UIViewController, CLLocationManagerDelegate{
    ...
    func BuildHospitals(){
        ...
        //Belinson
        let belinson = Hospital(name: "Belinson", d_latitude: 32.08802780, d_longitude: 34.86752920)
        hospitals.append(belinson)
        
        //Meir
        let meir = Hospital(name: "Meir", d_latitude: 32.18157320, d_longitude: 34.89565530)
        hospitals.append(meir)
        
        //Hilel Yafe
        let hilelyafe = Hospital(name: "Hilel Yafe", d_latitude: 32.45201470, d_longitude: 34.89636150)
        hospitals.append(hilelyafe)
        
        //Tel Hashomer
        let telhashomer = Hospital(name: "Tel Hashomer", d_latitude: 32.04422800, d_longitude: 34.84176300)
        hospitals.append(telhashomer)
        
        //Wolfson
        let wolfson = Hospital(name: "Wolfson", d_latitude: 32.03534250, d_longitude: 34.76281740)
        hospitals.append(wolfson)
    }
    ...
    func GetOnlineData() {
        ...
        refHospitals = Database.database().reference().child("hospitals");
        refHospitals.observe(.value, with: { (snapshot) in
            if snapshot.childrenCount > 0 {
                
                for h in snapshot.children.allObjects as![DataSnapshot]{
                    if(j == 5){
                        break;
                    }
                    ...
                    let hospitalFirstLine = hospitalObject["hospital_FirstInLine"] as! String;
                    let hospitalLastLine = hospitalObject["hospital_LastInLine"] as! String;
                    let hospitalDoctors = hospitalObject["hospital_Doctors"] as! String;
                    let hospitalAvgApp = hospitalObject["hospital_Avg_app"] as! String;
                    let hospitalAvgCom = hospitalObject["hospital_Avg_com"] as! String;
                    let hospitalPark = hospitalObject["hospital_Park"] as! String;
